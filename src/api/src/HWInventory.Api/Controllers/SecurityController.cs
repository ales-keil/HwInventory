...
public class SecurityController : ApiControllerBase
{
    ...
    private readonly ISecurityAlertConfigurationStore _securityAlertConfigurationStore;
    private readonly ISecurityMetricsSnapshotService _securityMetricsSnapshotService;
    ...
    public SecurityController(
        ...
        IPasswordPolicyConfigurationStore passwordPolicyConfigurationStore,
        ISecurityAlertConfigurationStore securityAlertConfigurationStore,
        ISecurityMetricsSnapshotService securityMetricsSnapshotService) : base(dbContext)
    {
        ...
        _securityAlertConfigurationStore = securityAlertConfigurationStore;
        _securityMetricsSnapshotService = securityMetricsSnapshotService;
    }
    ...
    public async Task<ActionResult<SecuritySummaryResponse>> GetSecuritySummaryAsync(CancellationToken cancellationToken)
    {
        ...
        var alertsConfiguration = await _securityAlertConfigurationStore.GetAsync(cancellationToken);
        var snapshots = await _securityMetricsSnapshotService.GetRecentAsync(14, cancellationToken);
    ...
    [HttpGet("alerts")]
    public async Task<ActionResult<SecurityAlertConfigurationModel>> GetSecurityAlertsAsync(CancellationToken cancellationToken)
    {
        var configuration = await _securityAlertConfigurationStore.GetAsync(cancellationToken);
        return Ok(configuration);
    }

    [HttpPost("alerts")]
    public async Task<ActionResult<SecurityAlertConfigurationModel>> UpdateSecurityAlertsAsync([FromBody] SecurityAlertConfigurationRequest request, CancellationToken cancellationToken)
    {
        if (request.MinimumTwoFactorAdoptionPercentage is < 0 or > 100)
        {
            return BadRequest(new ProblemDetails
            {
                Title = "Neplatné procento 2FA",
                Detail = "Hodnota musí být v rozmezí 0 až 100 %"
            });
        }

        if (request.LockedAccountThreshold < 0 || request.PendingResetThreshold < 0)
        {
            return BadRequest(new ProblemDetails
            {
                Title = "Neplatné prahové hodnoty",
                Detail = "Prahové hodnoty nesmí být záporné."
            });
        }

        var sanitizedEmails = (request.NotificationEmails ?? Array.Empty<string>())
            .Where(x => !string.IsNullOrWhiteSpace(x))
            .Select(x => x.Trim())
            .Distinct(StringComparer.OrdinalIgnoreCase)
            .ToArray();

        var previous = await _securityAlertConfigurationStore.GetAsync(cancellationToken);
        var update = new SecurityAlertConfigurationUpdate(
            request.Enabled,
            request.MinimumTwoFactorAdoptionPercentage,
            request.LockedAccountThreshold,
            request.PendingResetThreshold,
            sanitizedEmails);

        var saved = await _securityAlertConfigurationStore.SaveAsync(update, cancellationToken);

        AddAuditLog(
            "Security.Alerts",
            Guid.Empty,
            "Update",
            "Aktualizace bezpečnostních alertů",
            new
            {
                Before = new
                {
                    previous.Enabled,
                    previous.MinimumTwoFactorAdoptionPercentage,
                    previous.LockedAccountThreshold,
                    previous.PendingResetThreshold,
                    Emails = previous.NotificationEmails
                },
                After = new
                {
                    saved.Enabled,
                    saved.MinimumTwoFactorAdoptionPercentage,
                    saved.LockedAccountThreshold,
                    saved.PendingResetThreshold,
                    Emails = saved.NotificationEmails
                }
            });
    ...
    public async Task<ActionResult<IReadOnlyList<SecurityMetricSnapshotModel>>> GetSecurityMetricsHistoryAsync([FromQuery] int days = 30, CancellationToken cancellationToken = default)
    {
        var window = Math.Clamp(days, 1, 180);
        var snapshots = await _securityMetricsSnapshotService.GetRecentAsync(window, cancellationToken);
        return Ok(snapshots);
    }

    [HttpPost("metrics/capture")]
    public async Task<ActionResult<SecurityMetricSnapshotModel>> CaptureSecurityMetricsAsync(CancellationToken cancellationToken)
    {
        var snapshot = await _securityMetricsSnapshotService.CaptureAsync(cancellationToken);
        AddAuditLog("Security.Alerts", Guid.Empty, "Update", "Manuální zachycení bezpečnostních metrik", snapshot);
        await DbContext.SaveChangesAsync(cancellationToken);
        return Ok(snapshot);
    }
    ...
    public record SecuritySummaryResponse(
        ...
        bool CriticalReady,
        SecurityAlertConfigurationModel AlertsConfiguration,
        SecurityMetricSnapshotModel? LatestSnapshot);

    public record SecurityChangeMetadata(DateTime? TimestampUtc, string? Actor);

    public record SecurityAlertConfigurationRequest(
        bool Enabled,

...
public class ServersController : ApiControllerBase
{
    ...
    public async Task<ActionResult<PagedResult<Server>>> GetServersAsync(
    ...
    [Authorize(Policy = AuthorizationPolicies.ServersManage)]
    public async Task<IActionResult> RetireServersBatchAsync([FromBody] BatchActionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var scope = await ResolveDataScopeAsync(cancellationToken);
        var servers = await DbContext.Servers.Where(x => request.Ids.Contains(x.Id)).ToListAsync(cancellationToken);
    ...
    [Authorize(Policy = AuthorizationPolicies.ServersManage)]
    public async Task<IActionResult> RestoreServersBatchAsync([FromBody] BatchActionRequest request, CancellationToken cancellationToken = default)
    {
        if (request.Ids.Count == 0)
        {
            return BadRequest("No identifiers supplied.");
    ...
    [Authorize(Policy = AuthorizationPolicies.ServersManage)]
    public async Task<IActionResult> DeleteServersBatchAsync([FromBody] BatchActionRequest request, CancellationToken cancellationToken = default)
    {
        if (request.Ids.Count == 0)
        {
            return BadRequest("No identifiers supplied.");
        ...
        DbContext.Servers.RemoveRange(servers.Where(server => !scope.HasLocationRestrictions || scope.LocationIds.Contains(server.LocationId)));
        await DbContext.SaveChangesAsync(cancellationToken);
        return NoContent();
    }

...
public class WorkstationsController : ApiControllerBase
{
    ...
    public async Task<ActionResult> HandoverAsync(Guid id, [FromBody] HandoverRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var locationNames = await DbContext.DictionaryEntries
            .Where(x => x.DictType == "Location" && (x.Id == oldLocationId || x.Id == request.NewLocationId))
            .ToListAsync(cancellationToken);

        var oldLocationName = locationNames.FirstOrDefault(x => x.Id == oldLocationId)?.Value;
        var newLocationName = locationNames.FirstOrDefault(x => x.Id == request.NewLocationId)?.Value;
        ...
        var handoverOptions = new WorkstationHandoverOptions(
        ...
        var result = await _handoverService.ProcessAsync(entity, handoverOptions, cancellationToken);

        AddAuditLog(
            nameof(Workstation),
            entity.Id,
            "Handover",
            $"Předání na {request.NewOwnerDisplayName ?? "neuvedeno"} ({newLocationName ?? request.NewLocationId.ToString()}) – e-mail {(result.EmailSent ? "odeslán" : "neodeslán")}",
            new
            {
        ...
        return Ok(new { result.EmailSent, result.Message });
    }

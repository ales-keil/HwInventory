...
using HWInventory.Infrastructure.Reports;
...
public static class DependencyInjection
{
    public static IServiceCollection AddInfrastructure(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddAuthorization(options =>
        {
            ...
            options.AddPolicy(AuthorizationPolicies.ImportExportManage, policy =>
                policy.RequireRole(
                    ...
                    SystemRoleNames.AppAdmin,
                    SystemRoleNames.Helpdesk));
            options.AddPolicy(AuthorizationPolicies.ReportsManage, policy =>
                policy.RequireRole(SystemRoleNames.SuperAdmin));
        });
        ...
        services.AddScoped<IPrintingConnectorStore, PrintingConnectorStore>();
        ...
        services.AddScoped<ISecurityAlertConfigurationStore, SecurityAlertConfigurationStore>();
        services.AddScoped<ISecurityMetricsSnapshotService, SecurityMetricsSnapshotService>();
        ...
        services.AddQuartz(q =>
        {
            ...
            var reportJobKey = new JobKey("ReportJobProcessor");
            q.AddJob<ReportJobProcessor>(options => options.WithIdentity(reportJobKey));
            q.AddTrigger(trigger => trigger
                .ForJob(reportJobKey)
                .WithIdentity("ReportJobProcessor-trigger")
                .WithSimpleSchedule(x => x.WithIntervalInMinutes(10).RepeatForever()));

            var securityMetricsJobKey = new JobKey("SecurityMetricsSnapshotJob");
            q.AddJob<SecurityMetricsSnapshotJob>(options => options.WithIdentity(securityMetricsJobKey));
            q.AddTrigger(trigger => trigger
                .ForJob(securityMetricsJobKey)
                .WithIdentity("SecurityMetricsSnapshotJob-trigger")

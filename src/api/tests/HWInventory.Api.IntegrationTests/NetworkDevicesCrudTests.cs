...
public class NetworkDevicesCrudTests : IntegrationTestBase
{
    ...
    [Fact]
    public async Task NetworkDeviceFiltering_ReturnsExpectedSubset()
    {
        var switchType = await EnsureDictionaryEntryAsync("DeviceType", "SW", "Switch");
        var routerType = await EnsureDictionaryEntryAsync("DeviceType", "RT", "Router");
        var hqLocation = await EnsureDictionaryEntryAsync("Location", "HQ", "Headquarters");
        var branchLocation = await EnsureDictionaryEntryAsync("Location", "BR", "Branch");

        async Task<Guid> CreateDeviceAsync(string name, string inventory, Guid typeId, Guid locationId)
        {
            var request = new NetworkDeviceRequestDto(
                Name: name,
            ...
            var response = await Client.PostAsJsonAsync("/api/network-devices", request);
            response.EnsureSuccessStatusCode();
            var created = await response.Content.ReadFromJsonAsync<NetworkDevice>();
            Assert.NotNull(created);
            return created!.Id;
        }

        var switchId = await CreateDeviceAsync("Aggregation Switch", "NET-SW-01", switchType, hqLocation);
        var routerId = await CreateDeviceAsync("Edge Router", "NET-RT-01", routerType, branchLocation);

        var retireResponse = await Client.PostAsync($"/api/network-devices/{routerId}/retire", null);
        retireResponse.EnsureSuccessStatusCode();

        var searchResponse = await Client.GetFromJsonAsync<PagedResponse<NetworkDevice>>($"/api/network-devices?page=1&size=50&search=Aggregation");
        Assert.NotNull(searchResponse);
        Assert.Single(searchResponse!.Items);
        Assert.Equal(switchId, searchResponse.Items[0].Id);

        var statusResponse = await Client.GetFromJsonAsync<PagedResponse<NetworkDevice>>($"/api/network-devices?page=1&size=50&status=Retired");
        Assert.NotNull(statusResponse);
        Assert.Single(statusResponse!.Items);
        Assert.Equal(routerId, statusResponse.Items[0].Id);

        var typeResponse = await Client.GetFromJsonAsync<PagedResponse<NetworkDevice>>($"/api/network-devices?page=1&size=50&deviceTypeId={switchType}");
        Assert.NotNull(typeResponse);
        Assert.Single(typeResponse!.Items);
        Assert.Equal(switchId, typeResponse.Items[0].Id);

        var locationResponse = await Client.GetFromJsonAsync<PagedResponse<NetworkDevice>>($"/api/network-devices?page=1&size=50&locationId={branchLocation}");
        Assert.NotNull(locationResponse);
        Assert.Single(locationResponse!.Items);
        Assert.Equal(routerId, locationResponse.Items[0].Id);
    }

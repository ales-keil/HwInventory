...
export interface SecuritySummary {
  ...
  alertsConfiguration: SecurityAlertConfiguration;
  latestSnapshot?: SecurityMetricSnapshot | null;
}

export interface SecurityAlertConfiguration {
  enabled: boolean;
  minimumTwoFactorAdoptionPercentage: number;
  lockedAccountThreshold: number;
  pendingResetThreshold: number;
  notificationEmails: string[];
}

export interface SecurityAlertConfigurationRequest extends SecurityAlertConfiguration {}

export interface SecurityMetricAlert {
  code: string;
  message: string;
  severity: 'warning' | 'error' | string;
}

export interface SecurityMetricSnapshot {
...
export const getSecurityAlerts = async () => {
  const { data } = await apiClient.get<SecurityAlertConfiguration>('/security/alerts');
  return data;
};

export const updateSecurityAlerts = async (payload: SecurityAlertConfigurationRequest) => {
  const { data } = await apiClient.post<SecurityAlertConfiguration>('/security/alerts', payload);
  return data;
};

export const getSecurityMetricsHistory = async (days = 30) => {
  const { data } = await apiClient.get<SecurityMetricSnapshot[]>(`/security/metrics/history`, {
    params: { days }
  });
  return data;
};

export const captureSecurityMetrics = async () => {
  const { data } = await apiClient.post<SecurityMetricSnapshot>('/security/metrics/capture');
  return data;
};

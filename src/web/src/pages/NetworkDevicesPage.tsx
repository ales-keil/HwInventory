...
import {
  ...
  getNetworkDevices,
  retireNetworkDevice,
  retireNetworkDevices,
  restoreNetworkDevice,
  restoreNetworkDevices,
  updateNetworkDevice
...
export const NetworkDevicesPage = () => {
  ...
  const [filters, setFilters] = useState<DeviceFilterState>(() => ({ ...defaultFilters }));
  ...
  const loadDevices = async (pageIndex = page, overrideFilters?: DeviceFilterState) => {
    ...
    try {
      ...
      const response = await getNetworkDevices(pageIndex, pageSize, {
        ...
        deviceTypeId: appliedFilters.deviceTypeId
      });
      setDevices(response.items);
      setTotalCount(response.totalCount);
      setPage(response.page);
      setSelected((current) => current.filter((id) => response.items.some((device) => device.id === id)));
      setError(null);
    } catch (err) {
  ...
  const handleBulkAction = async (action: 'retire' | 'restore' | 'delete') => {
    ...
    if (action === 'delete' && !window.confirm(`Opravdu chcete smazat ${selected.length} vybraných zařízení?`)) {
      return;
    }

    try {
      if (action === 'retire') {
        await retireNetworkDevices(selected);
      } else if (action === 'restore') {
        await restoreNetworkDevices(selected);
      } else {
        await deleteNetworkDevices(selected);
      }
  ...
  const allVisibleSelected = devices.length > 0 && devices.every((device) => selected.includes(device.id));

  const toggleSelectAllVisible = () => {
    if (allVisibleSelected) {
      setSelected((current) => current.filter((id) => !devices.some((device) => device.id === id)));
    } else {
      setSelected((current) => {
        const additions = devices.map((device) => device.id).filter((id) => !current.includes(id));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <HelpTooltip manualPath="inventory.html" label="Otevřít kapitolu nápovědy k síťovým zařízením" inline />
      </div>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div className="flex flex-col gap-1">
            <label htmlFor="net-filter-search" className="text-xs font-semibold uppercase text-slate-500">
              Hledat
            </label>
            <input
              id="net-filter-search"
              name="search"
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="net-filter-status" className="text-xs font-semibold uppercase text-slate-500">
              Status
            </label>
            <select
              id="net-filter-status"
              name="status"
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="net-filter-type" className="text-xs font-semibold uppercase text-slate-500">
              Typ zařízení
            </label>
            <select
              id="net-filter-type"
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="net-filter-location" className="text-xs font-semibold uppercase text-slate-500">
              Lokalita
            </label>
            <select
              id="net-filter-location"
              name="locationId"
      ...
      {selected.length > 0 && (
        <div className="flex items-center justify-between rounded border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-500 dark:bg-amber-900/40 dark:text-amber-100">
          <span>Vybráno {selected.length} zařízení.</span>
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
            ...
            <tr>
              <th className="px-4 py-3">
                <input
                  ...
                  checked={devices.length > 0 && allVisibleSelected}
            ...
            {devices.map((device) => (
              <tr key={device.id} className="hover:bg-slate-50 dark:hover:bg-slate-800">
                <td className="px-4 py-3">
                  <input
                    type="checkbox"
                    aria-label={`Vybrat ${device.name}`}
                    checked={selected.includes(device.id)}
                    onChange={() => toggleSelect(device.id)}
                  />
                </td>
                <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{device.name}</td>
                <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{dictionaryLabel('DeviceType', device.deviceTypeId)}</td>
                <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{dictionaryLabel('Location', device.locationId)}</td>
                <td className="px-4 py-3">

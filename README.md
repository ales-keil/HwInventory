...
> **Status:** the solution is **not** production-ready. The current codebase now boots with SQL Server, ASP.NET Core Identity (including TOTP 2FA scaffolding), baseline policy-based RBAC guard rails with per-user location/department scoping, persistent audit logging, Quartz.NET job scheduling, a label rendering engine, and a React admin shell with dark/light theming. Security integrations now cover LDAP/AD test/dry-run endpoints, dynamic OIDC configuration, configurable password policies, CAPTCHA and SMS connector management, SMTP connector management with test e-mail delivery, AD group-to-role mappings, middleware-backed session tracking with remote logout APIs, plus dedikované React stránky „Security Settings“ a „Email (SMTP)“ pro správu OIDC/LDAP/SSPR/CAPTCHA/SMS/politik hesel, aktivních relací i SMTP konektoru. Součástí je také první iterace průvodce prvním spuštěním, která agreguje bezpečnostní kroky. Nejnovější práce doplnila katalog konektorů s UI „Konektory“, auditované observability nastavení (log level, /health, /metrics, OTLP endpoint) s novým API `/api/observability/*`, React stránku „Observabilita“ a middleware pro X-Correlation-Id i Prometheus-kompatibilní metriky. Aktuální iterace přidala modul „Backup & Restore“ s REST API `/api/maintenance/*`, Quartz úlohou pro plánované zálohy, šifrované ZIP archivy, SMTP notifikace a React stránku „Backup & Restore“, a nově také modul „Import/Export“ s API `/api/exports/*` a `/api/imports/*`, Quartz procesory exportních i importních jobů a rozšířenou React stránkou „Import/Export“ v nastavení. However, the majority of domain workflows (např. importní procesy, plánované reporty, aktualizace, další konektory a onboarding wizard) are still outstanding—see [`docs/implementation-plan.md`](docs/implementation-plan.md) for the full backlog.

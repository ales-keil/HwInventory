...
> 📘 **Nápověda & manuál:** Webové rozhraní načítá HTML kapitoly z adresáře [`docs/manual`](docs/manual). Sekce „Help Center“ v administraci i kontextové nápovědy ve formulářích využívají tyto soubory, proto je při release nutné balíček rozšířit také o aktuální HTML kapitoly.

> **Status:** the solution is **not** production-ready. The current codebase now boots with SQL Server, ASP.NET Core Identity (including TOTP 2FA scaffolding), baseline policy-based RBAC guard rails with per-user location/department scoping, persistent audit logging, Quartz.NET job scheduling, a label rendering engine, and a React administration experience with dark/light theming. Security integrations cover LDAP/AD test/dry-run endpoints, hardened HTTPS+PKCE OIDC configuration, configurable password policies with history/lockout enforcement, CAPTCHA and SMS connector management, per-user/per-IP SSPR throttling with configurable windows, SMTP connector management with test e-mail delivery, storage connector configuration (local disk/SMB/S3) with inline health checks, SFTP/FTPS connector configuration with secret rotation and health metadata, **printing connector management (RAW 9100/fronta) with auditované testy a sdílenými tajemstvími**, webhook connector configuration with signed test dispatch, AD group-to-role mappings, middleware-backed session tracking with remote logout APIs, a security readiness summary API/UI (blocking the onboarding wizard until all critical connectors are configured), scheduled security metric snapshots with configurable alert thresholds, historical trend visualisation, and e-mail notifications, plus dedicated React pages „Security Settings“, „Email (SMTP)“, „Webhooks“, „Konektory“, „Úložiště“, „SFTP / FTPS“, „**Tisk**“, „Observabilita“, „Backup & Restore“, „Import/Export“, „Reporty & Schedules“, „Aktualizace“, „Předávací protokoly“ a „Help Center“. Inventární moduly nyní zahrnují plnohodnotné React stránky pro Servery, Síťová zařízení, Pracovní stanice, Audit, Číselníky, Štítky a Moduly s CRUD formuláři, stránkovanými tabulkami, fulltextem a filtry (status, prostředí/typ, lokalita), hromadnými akcemi a integrací s číselníky a renderováním štítků. Na klíčových stránkách inventáře a nastavení se zároveň objevují kontextová tlačítka „Nápověda“, která otevírají příslušné kapitoly manuálu pro rychlou orientaci administrátorů. Workflow předání pracovních stanic nyní generuje QuestPDF protokol, odesílá e-mail s tlačítky „Přijmout“/„Odmítnout“ a po potvrzení příjemcem automaticky přepíše vlastníka/umístění (Super Admin může převod vynutit bez schválení). Úvodní dashboard načítá inventární statistiky, bezpečnostní ukazatele (2FA adopce, uzamčené účty, aktivní relace) a nejnovější auditní záznamy, přičemž stejné metriky jsou k dispozici i ve formátu Prometheus `/metrics`. Importní pipeline podporuje CSV/XLSX, konfliktní strategie a mapování sloupců, zatímco modul „Reporty & Schedules“ po každém dokončeném běhu odesílá e-mailové notifikace definovaným příjemcům, umožňuje nastavovat šablony předmětu i těla zprávy (s tokeny typu {ReportName}, {StatusText}, {CompletedAt}) a volitelně přiložit vygenerovaný artefakt; reporty i exportní joby zároveň respektují JSON filtry (`[{"field","op","value"}]` nebo objektovou zkratku) s whitelistem podporovaných polí pro Servery/Síťové prvky/Workstations/Audit včetně vazeb na číselníky a datumových hranic; k dispozici je REST API i React stránka pro správu definic, historií běhů, ruční spouštění a stahování artefaktů. Nově je k dispozici builder (`scripts/builder.ps1`) a GitHub Actions workflow (`.github/workflows/build-and-package.yml`) generující All-in-One balík se SHA256 checksumem a po publikaci automaticky spouštějící PowerShell smoke test proti běžící aplikaci. Zbývají pokročilé workflow (rozšířené observability dashboardy, smoke/regresní testy, fyzické tiskové fronty napojené na zařízení, pokročilé reportingové exporty) – kompletní backlog viz [`docs/implementation-plan.md`](docs/implementation-plan.md). Pokud potřebujete rychle nasadit pouze lokální variantu s inventářem serverů/sítě/stanic, sledujte roadmapu v [`docs/minimal-deployment-plan.md`](docs/minimal-deployment-plan.md) a detailní postup v [`docs/minimal-deployment-guide.md`](docs/minimal-deployment-guide.md).
...
- `scripts/builder.ps1` – vytváří MERGE/ALLINONE balíček.
- `scripts/installer.ps1` – nasazení na IIS včetně vytvoření DB a šifrování seed admina.
- `scripts/smoke-test.ps1` – rychlý ověřovací skript CRUD inventáře.
- `scripts/new-release.ps1` – generuje skeleton release notes pro novou verzi.
...
* Rozšířit bezpečnostní governance o pokročilé dashboardy (hlavní homepage widgety, observability metriky), širší regresní pokrytí (LDAP sync, SSPR edge cases, CAPTCHA throttling) a návazná opatření na alerty (napojení na SIEM/on-call, detailní reporting).

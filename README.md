...
> **Status:** the solution is **not** production-ready. The current codebase now boots with SQL Server, ASP.NET Core Identity (including TOTP 2FA scaffolding), baseline policy-based RBAC guard rails with per-user location/department scoping, persistent audit logging, Quartz.NET job scheduling, a label rendering engine, and a React admin shell with dark/light theming. Security integrations now cover LDAP/AD test/dry-run endpoints, dynamic OIDC configuration, configurable password policies, CAPTCHA and SMS connector management, SMTP connector management with test e-mail delivery, AD group-to-role mappings, middleware-backed session tracking with remote logout APIs, plus dedikované React stránky „Security Settings“ a „Email (SMTP)“ pro správu OIDC/LDAP/SSPR/CAPTCHA/SMS/politik hesel, aktivních relací i SMTP konektoru. Součástí je také první iterace průvodce prvním spuštěním, která agreguje bezpečnostní kroky. Nejnovější iterace doplnila auditované nastavení předávacího protokolu, generování PDF (QuestPDF) a e-mailů při předání pracovních stanic, jejich archivaci v nové tabulce `WorkstationHandovers` a odpovídající React stránku „Workstations“ se zobrazením historie. Dále přibylo administrační UI „Předávací protokol“ a nová stránka „Konektory“ v sekci Settings, která nabízí katalog integračních konektorů (SMTP, SMS, LDAP, OIDC) včetně přepínání, testů a rotace tajemství. However, the majority of domain workflows (imports/exports, backups, updates, extended connectors, remaining onboarding steps, etc.) are still outstanding—see [`docs/implementation-plan.md`](docs/implementation-plan.md) for the full backlog.

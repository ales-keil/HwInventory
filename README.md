...
> **Status:** the solution is **not** production-ready. The current codebase now boots with SQL Server, ASP.NET Core Identity (including TOTP 2FA scaffolding), baseline policy-based RBAC guard rails with per-user location/department scoping, persistent audit logging, Quartz.NET job scheduling, a label rendering engine, and a React admin shell with dark/light theming. Security integrations now cover LDAP/AD test/dry-run endpoints, dynamic OIDC configuration, CAPTCHA + SMS-backed self-service password reset (SSPR) flows, and middleware-backed session tracking with remote logout APIs, přičemž změny nastavení OIDC/datových scopů i vzdálená odhlášení se ukládají do auditu. However, the majority of domain workflows (handover, imports/exports, backups, updates, connectors, onboarding wizard, etc.) are still outstanding—see [`docs/implementation-plan.md`](docs/implementation-plan.md) for the full backlog.
...
* Finalising security governance: surface LDAP/SSO mapping UI, expand audit surfacing for security settings, implement CAPTCHA provider setup screens, and wire 2FA/SSPR flows into the onboarding wizard.

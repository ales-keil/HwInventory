...
- **Import/Export jobs**
  - Export jobs: rozšířit stávající `/api/exports/*` o filtrace, plánované exporty, expiraci odkazů, notifikace s více konektory a generování XLSX.
  - Import jobs: rozšířit nově dostupný CSV import o validace, mapování sloupců, konfliktní strategie se zápisem (skip/update/create), audit diffy starých/nových hodnot, podporu XLSX, per-user limity, e-mail/SMS notifikace a napojení na asynchronní frontu importů.
- **Backup & restore**

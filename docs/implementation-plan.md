...
> _Progress update:_ Identity foundation, PKCE-enforced OIDC configuration, password history enforcement, security readiness summary, baseline regression tests, and security alerting with scheduled metric snapshots plus React dashboards are in place. Items below track the remaining enhancements required for full compliance.
- **Identity provider**
...
- **Feature pages**
  ...
  - Help Center integration (embedded manual viewer, contextual "?" tooltips linking to manual chapters). ✅ _(2025-10-16 – viewer + kontextová tlačítka na klíčových stránkách inventáře a nastavení)_
- **API integration**
...
- **CI/CD** ✅ _(baseline workflow delivered 2025-10-09)_
  - Autor GitHub Actions workflows for build, test (API + web), publish artefacts, generate All-in-One ZIP with SHA256 manifest, and smoke tests. ✅ _(Nově pipeline spouští PowerShell smoke test nad publikovaným API; zbývá FE lint/test, rozšířené regresní scénáře a release publikace)_
- **All-in-One artefact**
...
- **Documentation**
  - Produce manual.pdf, release notes template updates, user guide, deployment guide, troubleshooting, rollback instructions. ✅ _(release notes scaffold: `docs/release-notes-template.md` + `scripts/new-release.ps1`)_
